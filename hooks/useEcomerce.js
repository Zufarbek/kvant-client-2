    ...
    return {
        ...
        getProducts: async (payload, group = '') => {
            ...
            if (payload && payload.length > 0) {
                ...
                let data = await payload.map(async(item) => {
                    return  await ProductRepository.getProductsById(item.id)
                });

                Promise.all(data).then((responseData) => {
